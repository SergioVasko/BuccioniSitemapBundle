...
`buccioni:sitemap:populate`. Foreach group of urls that you want to add to the
...
use Buccioni\Bundle\SitemapBundle\Provider\UrlProviderInterface;
...
class ExampleUrlProvider implements UrlProviderInterface {
    ...
    {
        $exampleUrl = $this->router->generate('buccioni_news_show');
        $url = new Url();
...
services:
    your_bundle.sitemap.provider.sample:
        ...
        arguments: [@router]
        tags:
            -  { name: buccioni_sitemap.provider }
```
...
After providers are in place and registered, time to run the generation command:
    ...
    > php app/console buccioni:sitemap:populate
